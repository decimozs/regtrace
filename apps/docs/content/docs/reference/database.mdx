...
```mermaid
flowchart LR
  Run[regtrace run] --> File[JSON File]
  File -->|Primary| Storage[Source of Truth]
  Run --> DB{DB Enabled?}
  DB -->|Yes| Insert[SQLite Insert]
  DB -->|No| Skip[Skip DB Write]
  Insert --> Cache[Derived Cache]
  Rebuild[db rebuild] -->|Reads all files| RebuildInsert[Batch Insert]
  RebuildInsert --> Cache
```
