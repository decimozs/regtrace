...
<Steps>
  <Step>
    ...
    ```
    https://github.com/decimozs/regtrace/releases/latest/download/regtrace-linux-x64-{platform}
    ```
        ...
        - name: Download Regtrace
          run: |
            curl -L -o /usr/local/bin/regtrace \
              https://github.com/decimozs/regtrace/releases/latest/download/regtrace-linux-x64
...
jobs:
  evaluate:
    ...
    steps:
      ...
      - name: Run evaluation
        run: regtrace run --ci --bail --generate
        env:
...
evaluate:
  ...
  before_script:
    ...
    - curl -L -o /usr/local/bin/regtrace
      "https://github.com/decimozs/regtrace/releases/$REGTRACE_VERSION/download/regtrace-linux-x64"
    - chmod +x /usr/local/bin/regtrace
  script:
    - regtrace run --ci
  artifacts:
