...
- `.gitignore` — with `.regtrace/runs/` entry
...
Edit `golden-sets/qa.yaml` and fill in `actual_output` for both cases. Without
an LLM provider, Regtrace falls back to heuristic scoring — for meaningful
results, provide model responses:
