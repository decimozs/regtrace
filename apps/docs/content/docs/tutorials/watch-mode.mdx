...
sequenceDiagram
  ...
  participant Watch as Regtrace Watch
  participant Eval as Evaluation Engine

  Dev->>Watch: Start watch mode
  Note over Watch: Watching golden set files
  Dev->>Dev: Edit golden set file
  Dev->>Dev: Save file
  Watch->>Watch: Debounce 500ms
  Watch->>Eval: Trigger re-run
  Eval->>Watch: Evaluation results
  Watch->>Dev: Display results
```
